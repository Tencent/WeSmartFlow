...
- **用户画像**：通过 UserProfileStore 读取并注入用户画像。
"""
...
from ..memory.profile import FileUserProfileStore, UserProfileStore
...
class ContextBuilderWithProfileAndSkill(BaseContextBuilder):
    ...
    2. **用户画像**：从 ``UserProfileStore`` 读取完整画像，注入为独立段落。
    3. **Always Skills**：标记了 ``always: true`` 的技能，始终注入上下文。
    4. **Skills 摘要**：所有技能的 XML 摘要，供 Agent 按需读取。

    Args:
        workspace_dir:        用户数据根目录，用于定位 ``agent.md``、``profile/``、``skills/`` 等子路径。
        extra_identity_files:  额外的自我定义文件路径列表（绝对路径），可选。
        profile_store:         用户画像存储实例，默认使用 ``FileUserProfileStore``。
    ...
    Example::
        ...
        # 对话结束后更新用户画像
        await ctx.profile_store.consolidate(messages, llm)
    """
    ...
    def __init__(
        ...
        self.profile_store: UserProfileStore = profile_store or FileUserProfileStore(
            profile_file=workspace_dir / "profile" / "profile.md",
            history_file=workspace_dir / "profile" / "history.md",
        )
    ...
    def build_system_prompt(
        ...
        **kwargs: Any,
        ...
        # 3. 用户画像
        profile_text = self.profile_store.read_full()
        if profile_text:
            parts.append(f"## 用户画像\n\n{profile_text}")
    ...
    def build_messages(
        ...
        **kwargs: Any,
        ...
        if not history:
            history = []
        system_prompt = self.build_system_prompt(skill_names=skill_names, **kwargs)
